...
char parse_char(const cv::Point& point){
	...
	if(0 <= point.y && point.y <= 130){
		std::vector<char> arr = {'1', '2', '3', '4', '5', '6', '7', '8', '9', '0', '\b', '\b'} ;
		set_char(arr, 0, ch, point);
	}else if(150 <= point.y && point.y <= 280){
		std::vector<char> arr = {'Q', 'W', 'E', 'R', 'T', 'Y', 'U', 'I', 'O', 'P', '\b', '\b'} ;
		set_char(arr, 0, ch, point);
...
void display_buf(const std::vector<std::string>& typedStrings, const std::vector<char>& buff){
	std::cout << "\x1B[2J\x1B[H";
	if(typedStrings.size() > 0){
		for(std::string s : typedStrings){
			std::cout << "* " << s << '\n';
		}
	}
	if(buff.size() > 0){
...
int main(int argc, char* argv[]) {
	try {
		...
		std::vector<std::string> typedStrings;
		bool flipImage = true;
		...
		do {
			...
			if(ch == '\b' && buff.size() > 0){
				...
				std::string enteredStr(buff.begin(), buff.end());
				if(typedStrings.size() > 10){
					typedStrings.erase(typedStrings.begin());
				}
				if(trim(enteredStr).compare("") != 0){
					typedStrings.push_back(enteredStr);
				}
				buff.clear();
			}
